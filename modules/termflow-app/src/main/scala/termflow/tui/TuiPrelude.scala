...
object TuiPrelude:
  ...
  // 1-based coordinate syntax (`2.x` / `10.y`) and fluent `Text` builders
  // (`"hi".text`) live in [[ScreenPrelude]] so screen-only callers can use
  // them without depending on the app layer. They are re-exported here
  // verbatim so existing `import TuiPrelude.*` call sites keep working.
  export ScreenPrelude.x
  export ScreenPrelude.y
  export ScreenPrelude.text
