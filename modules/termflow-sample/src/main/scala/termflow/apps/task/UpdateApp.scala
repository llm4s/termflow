...
object UpdateApp:
  ...
  def apply(m: Model, msg: Msg): Tui[Model, Msg] =
    msg match
      ...
      case Msg.Add(id) =>
        if m.tasks.contains(id) then m.copy(renderList = AppErrorMsg(s"$id already exists")).tui
        else
      ...
      case Msg.Remove(id) =>
        if !m.tasks.contains(id) then m.copy(renderList = AppErrorMsg(s"$id does not exist")).tui
        else
  ...
  private def updateTaskStatus(m: Model, id: TaskId, status: TaskStatus): Model =
    if !m.tasks.contains(id) then m.copy(renderList = AppErrorMsg(s"$id does not exist"))
    else
  ...
  def replaceInList[A](list: List[A])(predicate: A => Boolean, replacement: A => A): List[A] =
    list.map(item => if predicate(item) then replacement(item) else item)
